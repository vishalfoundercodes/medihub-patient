...
import {
  ...
  Star, ShoppingCart, Calendar, CheckCircle, Plus, Minus
} from 'lucide-react';
...
function WishlistCard({ item, onRemove, cart, onAddToCart, onRemoveFromCart }) {
  const navigate = useNavigate();

  // For tests: qty in selected array
  const testQty = item.category === 'test' ? (cart.tests.find((t) => t.id === item.id) ? 1 : 0) : 0;
  // For medicines: qty count
  const medQty = item.category === 'medicine' ? (cart.medicines[item.id] || 0) : 0;
  ...
  const cfg = categoryConfig[item.category] || categoryConfig.test;
  const Icon = cfg.icon;
    ...
    <div className="bg-white rounded-2xl border border-[var(--color-border)] hover:shadow-md transition-all duration-200 overflow-hidden group">
      ...
      <div className="relative overflow-hidden bg-[var(--color-bg-section)]">
        ...
        <button
          ...
          className="absolute top-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:bg-red-50 hover:text-red-500 transition-all"
        >
      ...
      <div className="p-4">
        ...
        {item.category === 'test' && (
          <>
            <p className="text-xs text-[var(--color-text-secondary)] mb-2">{item.includes}</p>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xl font-bold text-[var(--color-text-dark)]">₹{item.price}</span>
              <span className="text-sm text-[var(--color-text-secondary)] line-through">₹{item.original}</span>
              <span className="text-xs font-bold text-[var(--color-success)] bg-[var(--color-success-bg)] px-2 py-0.5 rounded-full">{item.discount}% OFF</span>
            </div>
          </>
        )}
        ...
        {item.category === 'medicine' && (
          <>
            <p className="text-xs text-[var(--color-text-secondary)] mb-2">{item.strip}</p>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xl font-bold text-[var(--color-text-dark)]">₹{item.price}</span>
              <span className="text-sm text-[var(--color-text-secondary)] line-through">₹{item.original}</span>
              <span className="text-xs font-bold text-[var(--color-success)] bg-[var(--color-success-bg)] px-2 py-0.5 rounded-full">{item.discount}% OFF</span>
        ...
        {item.category === 'doctor' && (
          <>
            <p className="text-xs text-[var(--color-text-secondary)] mb-2">{item.specialty} • {item.experience} Yrs Exp</p>
            <div className="flex items-center gap-2 mb-2">
              <Star className="w-3.5 h-3.5 fill-yellow-400 text-yellow-400" />
              <span className="text-sm font-bold text-[var(--color-text-dark)]">{item.rating}</span>
              <span className="text-xs text-[var(--color-text-secondary)]">({item.reviews}+ reviews)</span>
            </div>
            <p className="text-xl font-bold text-[var(--color-text-dark)] mb-1">₹{item.fee}</p>
          </>
        )}
        ...
        {item.category === 'doctor' && (
          <button
            onClick={() => navigate(`/book-appointment/${item.id}`)}
            className="w-full mt-3 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white transition-all"
          >
        ...
        {item.category === 'test' && (
          testQty > 0 ? (
            <button
              ...
              className="w-full mt-3 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold bg-[var(--color-success)] hover:bg-green-600 text-white transition-all"
            >
            ...
            <button
              ...
              className="w-full mt-3 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold bg-blue-50 hover:bg-[var(--color-primary)] text-[var(--color-primary)] hover:text-white border border-blue-100 hover:border-[var(--color-primary)] transition-all"
            >
...
export default function Wishlist() {
  ...
  // Tests local cart (lab checkout)
  const [testCart, setTestCart] = useState([]);

  const handleAddToCart = (type, item) => {
    if (type === 'test') {
      setTestCart((p) => p.find((t) => t.id === item.id) ? p : [...p, item]);
    } else {
      addToCart(item);
    }
  };
  ...
  const handleRemoveFromCart = (type, item) => {
    if (type === 'test') {
      setTestCart((p) => p.filter((t) => t.id !== item.id));
    } else {
  ...
  // Pass correct qty to card
  const getCartForCard = () => ({ tests: testCart, medicines: cart });
  ...
  const counts = {
    ...
    doctor:   items.filter((i) => i.category === 'doctor').length,
  };
        ...
        {filtered.length > 0 ? (
          ...
          <div className="bg-white rounded-2xl border border-[var(--color-border)] p-20 text-center">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] mb-5">
              {activeTab === 'all'
                ? 'Save your favourite tests, medicines and doctors here'
                : `Browse ${activeTab}s and add them to your wishlist`}
            </p>
            <div className="flex gap-3 justify-center flex-wrap">
              <button onClick={() => window.location.href = '/lab-tests'} className="flex items-center gap-2 bg-[var(--color-primary)] text-white font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-[var(--color-primary-dark)] transition-all">
                <FlaskConical className="w-4 h-4" /> Browse Tests
              </button>
              <button onClick={() => window.location.href = '/doctors'} className="flex items-center gap-2 border border-[var(--color-border)] text-[var(--color-text-dark)] font-semibold px-5 py-2.5 rounded-xl text-sm hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition-all">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--color-text-secondary)]">
                  {cartMedCount > 0 ? `₹${cartTotal.toLocaleString()} total` : 'Added from wishlist'}
                </p>
              </div>
            </div>
            <button
