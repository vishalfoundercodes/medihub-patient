...
import { X, Phone, Shield, CheckCircle, LogIn, Loader2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

// Simulated registered users
const REGISTERED_USERS = [
  { phone: '9999999999', name: 'Rahul Sharma', email: 'rahul@example.com' },
  { phone: '8888888888', name: 'Vishal', email: 'vishal@example.com' },
];
...
export default function LoginModal() {
  ...
  const handleSendOtp = () => {
    ...
    setTimeout(() => {
      ...
      console.log(`OTP for ${phone}: ${mock}`); // dev only
      setStep(STEPS.OTP);
      setResendTimer(30);
      setLoading(false);
    }, 1000);
  ...
  const handleLogin = () => {
    const found = REGISTERED_USERS.find((u) => u.phone === phone);
    if (found) {
      setStep(STEPS.SUCCESS);
      setTimeout(() => login(found), 1200);
    } else {
  ...
  const handleResend = () => {
    const mock = '111111';
    setGeneratedOtp(mock);
    console.log(`Resent OTP for ${phone}: ${mock}`);
    setOtp(['', '', '', '', '', '']);
    setOtpVerified(false);
    setError('');
    setResendTimer(30);
        ...
        <div className="px-4 md:px-8 pb-8 pt-4">
          ...
          {step === STEPS.OTP && (
            <div className="space-y-5">
              ...
              {/* Login button — enabled only after verify */}
              {otpVerified && (
                <button
                  onClick={handleLogin}
